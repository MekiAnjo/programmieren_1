...
public class Bruch {
	...
	public Bruch addiere(Bruch summand){
		int ergZaehler = zaehler * summand.getNenner() + summand.getZaehler() * nenner;
	...
	public Bruch subtrahiere(Bruch subtrahend){
		int ergZaehler = zaehler * subtrahend.getNenner() - subtrahend.getZaehler() * nenner;
	...
	public Bruch hoch(int exp){
		if(exp < 0)
			return this.hoch(-exp).kehrwert();

		if(exp == 0)
			return new Bruch(1, 1);
		else
			return this.multipliziere(this.hoch(exp - 1));		
	}
	...
	public String toString(){
		if (nenner != 1)
			...
			return Integer.valueOf(zaehler).toString(); //int in String umwandeln
	}
}
